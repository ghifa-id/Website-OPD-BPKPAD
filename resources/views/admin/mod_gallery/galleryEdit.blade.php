    ...
    <div class="container-fluid px-4">
        <div class="card my-4 shadow-lg border-0">
            ...
            <div class="card-body p-5">
                ...
                <form action="{{ route('update.gallery', $gallery->id_gallery) }}" method="POST"
                        ...
                        <select name="id_album" id="id_album" class="form-select" required>
                            @foreach ($album as $item)
                                <option value="{{ $item->id_album }}"
                                    {{ $gallery->id_album == $item->id_album ? 'selected' : '' }}>
                                    {{ $item->jdl_album }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="jdl_gallery" class="form-control" value="{{ $gallery->jdl_gallery }}"
                            required>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Upload Foto Baru (Opsional)</label><br>
                        <small class="text-muted">Kosongkan jika tidak ingin mengganti foto</small>
                        <input type="file" name="photo" class="form-control mt-2">
                        <div class="mt-3">
                            <strong>Foto Saat Ini:</strong><br>
                            <img src="{{ asset('asset/img_gallery/' . $gallery->gbr_gallery) }}" class="img-thumbnail mt-2"
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="keterangan" class="form-control" rows="3">{{ $gallery->keterangan }}</textarea>
                    </div>
                    ...
                    <div class="d-flex gap-3 pt-4 border-top">
                        ...
                        <a href="{{ route('gallery') }}" class="btn btn-outline-secondary btn-lg px-4 shadow-sm">
