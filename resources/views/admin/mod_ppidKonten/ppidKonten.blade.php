                ...
                <div class="row align-items-center">
                    <div class="col">
                        <h6 class="mb-0 fw-semibold text-gray-700">
                            <i class="fas fa-images text-success me-2"></i> Daftar PPID
                        </h6>
                    </div>
                    <div class="col-auto">
                        <a href="{{ route('tambah_ppidKontenAdmin') }}" class="btn btn-success btn-sm px-3">
                            <i class="fas fa-plus me-1"></i> Tambah PPID
                        </a>
                    </div>
                </div>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0" id="albumTable">
                        <thead class="table-light">
                            <tr>
                                <th class="text-center" style="width: 50px;"><small
                                        class="text-muted fw-semibold">No</small></th>
                                <th style="min-width: 250px;"><small class="text-muted fw-semibold">Konten</small></th>
                                <th style="width: 100px;"><small class="text-muted fw-semibold">Link</small></th>
                                <th class="text-center" style="width: 100px;"><small
                                        class="text-muted fw-semibold">Tanggal</small></th>
                                <th class="text-center" style="width: 120px;"><small
                                        class="text-muted fw-semibold">Aksi</small></th>
                            </tr>
                        </thead>
                        <tbody>
                            @php
                            ...
                            @endphp
                            @forelse ($ppidkonten as $index => $row)
                                <tr class="album-row" data-status="{{ $row->aktif }}"
                                    data-title="{{ strtolower($row->judul) }}">
                                    <td class="text-center"><small class="text-muted">{{ $index + 1 }}</small></td>
                                    <td class="text-wrap" style="max-width: 300px;">{{ $jenisList[$row->jenis] ?? '-' }}
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center gap-2">
                                            <a href="{{ route('edit_ppidKontenAdmin', $row->id_ppidkonten) }}"
                                                class="btn btn-outline-success btn-sm px-2" title="Edit download">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <button type="button" class="btn btn-outline-danger btn-sm btn-delete px-2"
                                                data-id="{{ $row->id_ppidkonten }}"
                                                data-title="{{ $jenisList[$row->jenis] ?? 'Konten PPID' }}" title="Hapus">
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                deleteButtons.forEach(button => {
                    button.addEventListener('click', function() {
                        ...
                        deleteForm.action = "{{ url('/administrator/ppidKontenAdmin') }}/" + id;
                        deleteModal.show();
                    });
                });
            });
