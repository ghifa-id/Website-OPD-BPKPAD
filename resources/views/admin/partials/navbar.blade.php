<nav class="navbar navbar-main navbar-expand-lg px-0 mx-3 shadow-none border-radius-xl" id="navbarBlur" data-scroll="true">
    <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                <li class="breadcrumb-item text-sm">
                    <a class="opacity-5 text-success" href="{{ route('administrator.beranda') }}">Administrator</a>
                </li>
                @if (!empty($parentBreadcrumb))
                    <li class="breadcrumb-item text-sm">
                        <a class="opacity-5 text-success" href="{{ url('/administrator/' . $parentBreadcrumbUrl) }}">
                            {{ $parentBreadcrumb }}
                        </a>
                    </li>
                @endif
                <li class="breadcrumb-item text-sm text-success active" aria-current="page">
                    {{ $breadcrumb ?? 'beranda' }}
                </li>
            </ol>
        </nav>
        ...
        @endphp
        <!-- Navbar -->
        <nav class="navbar navbar-main navbar-expand-lg px-2 mx-2 shadow-none border-radius-xl bg-success text-white"
            style="height: 50px;">
                    ...
                    <ul class="navbar-nav d-flex align-items-center">
                        <li class="nav-item dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0 rounded-3 extra-small-dropdown animate-fade p-1"
                                ...
                                <!-- Profile Menu -->
                                <li class="py-0">
                                    <a class="dropdown-item d-flex align-items-center text-success small-text"
